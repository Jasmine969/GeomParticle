...
def _check_size_change(old, new, name_geo, name_prop):
    """
    Warn if a property value has been quantized (changed) during geometry construction.

    Args:
        old (float): Original property value.
        new (float): New property value after quantization.
        name_geo (str): Name of the geometry.
        name_prop (str): Name of the property.
    """
    filter_guide = " Filter this warning by filterwarnings('ignore',message='.*quantized.*')"
    if np.isclose(old, new):
        return
    warn(f"{name_geo}: {name_prop} quantized from {old: .4g} to {new: .4g}" + filter_guide)
